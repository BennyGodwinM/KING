...
def manual_diagnostic_run(episodes=5):
    """
    Manual diagnostic mode.

    You choose the action each step from the terminal:
        F = forward
        L = left
        R = right
        S = stop robot, then ask again
        Q = quit

    Every F/L/R step still goes through env.step(), so it prints and logs:
        reward_success
        reward_collision
        reward_avoidance
        reward_time
    ...
    try:
        for ep in range(episodes):
            print(f"\nMANUAL DIAGNOSTIC EPISODE {ep + 1}")
            obs, _ = env.reset()
            done = False

            while not done:
                print("\nCurrent observation:")
                print(f"  R_est = {float(obs[0]):.3f} m")
                print(f"  theta = {float(obs[6]):.2f} deg")
                print(f"  left_depth   = {float(obs[1]):.3f} m")
                print(f"  center_depth = {float(obs[2]):.3f} m")
                print(f"  right_depth  = {float(obs[3]):.3f} m")
                print(f"  front_invalid = {float(obs[4]):.3f}")
                print(f"  target_visible = {int(obs[5])}")
                print(f"  L/C/R invalid = {float(obs[7]):.3f} / {float(obs[8]):.3f} / {float(obs[9]):.3f}")

                cmd = input("Enter action [F/L/R], S=stop, Q=quit: ").strip().upper()

                if cmd == "Q":
                    env.stop_robot()
                    print("Manual diagnostic stopped by user.")
                    return

                if cmd == "S":
                    env.stop_robot()
                    print("Sent STOP. No reward step logged for S.")
                    continue

                if cmd not in manual_action_to_id:
                    print("Invalid command. Use F, L, R, S, or Q.")
                    continue

                action = manual_action_to_id[cmd]
                obs, reward, terminated, truncated, info = env.step(action)
                done = terminated or truncated

                print("\nStep result:")
                print(f"  reward_total = {reward:.3f}")
                print(f"  success = {terminated and not info.get('camera_fail', False)}")
                print(f"  terminated = {terminated}")
                print(f"  truncated = {truncated}")

            print("Episode ended. Start next episode or press Q on the next action prompt.")

        print(f"Manual diagnostic log saved to {LOG_PATH}")
