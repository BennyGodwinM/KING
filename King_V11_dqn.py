...
MODEL_PATH = "dqn_target_nav_model_6"
LOG_PATH = "dqn_step_log_6.csv"
...
class RealRobotDQNEnv(gym.Env):
    ...
    def step(self, action):
        ...
        if self.prev_theta_est_deg is not None:
            if abs(curr_theta_est_deg) > CENTERED_FORWARD_ANGLE_DEG:
            ...
            if curr_theta_est_deg < -CENTERED_FORWARD_ANGLE_DEG and action_char == "L":
                reward_angle += 0.005
            elif curr_theta_est_deg > CENTERED_FORWARD_ANGLE_DEG and action_char == "R":
                reward_angle += 0.005

        reward_angle = float(np.clip(reward_angle, -0.15, 0.15))
        ...
        if (
                action_char == "F"
                and not avoidance_active
                and info["center_danger"] < 0.25
                and abs(curr_theta_est_deg) < 5.0
        ):
            reward_forward += 0.10
